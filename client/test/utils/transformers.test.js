...
import { latLngToText, placeToLatLng, latLngToPlace ,placeToLatLngWithName , latLngToPlaceWithName} from '../../src/utils/transformers';

describe('transformers', () => {
    const latLng = { lat: 40.12345312, lng: 50.12532245 };
    const place = { latitude: '40.12345312', longitude: '50.12532245' };
    const latLngName = { name : "Place Name", lat: 40.12345312, lng: 50.12532245 };
    const placeName = { name: "Place Name", latitude: '40.12345312', longitude: '50.12532245' };
    ...
    it('placeToLatLngWithName converts correctly', () => {
        const convertedLatLng = placeToLatLngWithName(placeName);
        expect(convertedLatLng).toEqual(latLngName);
        const shouldBeNull = placeToLatLngWithName(null);
        expect(shouldBeNull).toBeNull();
        const shouldBeUndefined = placeToLatLngWithName(undefined);
        expect(shouldBeUndefined).toBeUndefined();
    });
    ...
    it('latLngToPlaceWithName converts correctly', () => {
        const convertedLatLng = latLngToPlaceWithName(latLngName);
        expect(convertedLatLng).toEqual(placeName);
        const shouldBeNull = latLngToPlaceWithName(null);
        expect(shouldBeNull).toBeNull();
        const shouldBeUndefined = latLngToPlaceWithName(undefined);
        expect(shouldBeUndefined).toBeUndefined();
    });
});
